...
export class BaseCustomControl extends BaseCustomWebComponentConstructorAppend {
    ...
    async connectedCallback() {
        ...
        // Apply visibility control if enabled in settings
        const settings = this.constructor[webuiCustomControlSymbol].control.settings;
        if (settings?.visibilityEnabled) {
            const visibilityConfig = {
                enabled: settings.visibilityEnabled,
                groups: settings.visibilityGroups || [],
                action: settings.visibilityAction || 'hide',
                redirectScreen: settings.visibilityRedirectScreen || ''
            };
            await visibilityService.applyVisibility(this, visibilityConfig);
        }
    ...
    disconnectedCallback() {
        ...
        // Remove visibility control from the custom control itself
        visibilityService.removeVisibility(this);

...
export class IobrokerWebuiSolutionExplorer extends BaseCustomWebComponentConstructorAppend {
    ...
    static style = css `
        * {
        ...
        .control-thumbnail-tooltip .tooltip-title {
            font-size: 11px;
            font-weight: bold;
            margin-top: 4px;
            color: #333;
    ...
    _createThumbnailTooltip() {
        ...
        this.shadowRoot.appendChild(this._thumbnailTooltip);
        console.log('✅ [Thumbnail] Tooltip element created');
    }
    async initialize(serviceContainer) {
        this.serviceContainer = serviceContainer;
        iobrokerHandler.objectsChanged.on(async (x) => {
            console.log('✅ [ObjectsChanged] Received objects changed event for type:', x.type);
            ...
            ContextMenu.show([{
                    ...
                    title: 'Copy ' + type, action: async () => {
                        ...
                        if (newName && newName != name) {
                            ...
                            await iobrokerHandler.saveObject(type, (dir ?? '') + '/' + newName, copy);
                            // Copy thumbnail if it's a control
                            if (type == 'control') {
                                try {
                                    const oldPath = (dir ?? '') + '/' + name;
                                    const newPath = (dir ?? '') + '/' + newName;
                                    const thumbnail = await iobrokerHandler.getThumbnail(oldPath);
                                    if (thumbnail) {
                                        await iobrokerHandler.saveThumbnail(newPath, new Blob([thumbnail.file], { type: 'image/png' }));
                                    }
                                }
                                catch (err) {
                                    console.warn('Error copying thumbnail:', err);
                                }
                            }
                        }
                    }
                    ...
                    title: 'Rename ' + type, action: async () => {
                        ...
                        if (newName && name != newName) {
                            ...
                            // Rename thumbnail if it's a control
                            if (type == 'control') {
                                try {
                                    const oldPath = (dir ?? '') + '/' + name;
                                    const newPath = (dir ?? '') + '/' + newName;
                                    ...
                                    if (thumbnail) {
                                        await iobrokerHandler.saveThumbnail(newPath, new Blob([thumbnail.file], { type: 'image/png' }));
                                        await iobrokerHandler.removeThumbnail(oldPath);
                                    }
                                }
                                catch (err) {
                                    console.warn('Error renaming thumbnail:', err);
                                }
                            }
                        }
                    }
                }, {
                    title: 'Remove ' + type, action: async () => {
                        if (confirm("are you sure?")) {
                            await iobrokerHandler.removeObject(type, (dir ?? '') + '/' + name);
                            // Remove thumbnail if it's a control
                            if (type == 'control') {
                                await iobrokerHandler.removeThumbnail((dir ?? '') + '/' + name);
                            }
                        }
                    }
                }, {
    ...
    _setupThumbnailHoverOnElement(element, controlName) {
        if (!element) {
            console.warn('❌ [Thumbnail] No element provided for', controlName);
            return;
        }
        console.log('✅ [Thumbnail] Hover listeners added for:', controlName);
        let hoverTimeout;
        element.addEventListener('mouseenter', async (e) => {
            console.log('🖱️ [Thumbnail] Mouse entered for:', controlName);
            hoverTimeout = setTimeout(async () => {
                console.log('⏱️ [Thumbnail] Timeout triggered, loading thumbnail for:', controlName);
                try {
                    const thumbnail = await iobrokerHandler.getThumbnail(controlName);
                    console.log('📦 [Thumbnail] Loaded thumbnail:', thumbnail ? 'SUCCESS' : 'NOT FOUND', 'for', controlName);
                    if (thumbnail && thumbnail.file) {
                        const blob = new Blob([thumbnail.file], { type: 'image/png' });
                        const url = URL.createObjectURL(blob);
                        this._thumbnailTooltip.innerHTML = `
                            <img src="${url}" alt="${controlName}" />
                            <div class="tooltip-title">${controlName}</div>
                        `;
                        const rect = element.getBoundingClientRect();
                        const hostRect = this.getBoundingClientRect();
                        this._thumbnailTooltip.style.left = (rect.right - hostRect.left + 10) + 'px';
                        this._thumbnailTooltip.style.top = (rect.top - hostRect.top) + 'px';
                        this._thumbnailTooltip.style.display = 'block';
        ...
        element.addEventListener('mouseleave', () => {
            ...
            this._thumbnailTooltip.style.display = 'none';
        });
    }
    _createGlobalNode() {
        return {
            ...
            this._tree = new Wunderbaum({
                ...
                render: (e) => {
                    if (e.isNew) {
                        ...
                        // Setup thumbnail hover for custom controls
                        if (e.node.data.data && e.node.data.data.type == 'control' && e.node.data.data.name) {
                            console.log('✅ [Thumbnail] Setting up hover for control:', e.node.data.data.name, 'on element:', span);
                            this._setupThumbnailHoverOnElement(span, e.node.data.data.name);
                        }
                    }
                },
                init: function (e) {

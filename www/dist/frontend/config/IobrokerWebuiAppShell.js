...
export class IobrokerWebuiAppShell extends BaseCustomWebComponentConstructorAppend {
    ...
    _setupVisibilityPanel() {
        console.log('🔧 [Visibility] Setting up panel...');
        
        // Use _getDomElement to access shadowRoot elements
        const visibilityDock = this._getDomElement('visibilityDock');
        if (!visibilityDock) {
            console.error('❌ [Visibility] visibilityDock not found!');
            return;
        }
        ...
        if (!visibilityPanel) {
            ...
            visibilityPanel.innerHTML = `
                <h3 style="margin:0 0 15px 0;font-size:14px;font-weight:bold;color:#333;">🔒 Visibility Control</h3>

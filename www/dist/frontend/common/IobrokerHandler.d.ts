...
export declare class IobrokerHandler implements VisualizationHandler {
    ...
    saveThumbnail(controlName: string, imageData: Blob): Promise<void>;
    getThumbnail(controlName: string): Promise<{
        mimeType: string;
        file: ArrayBuffer;
    } | null>;
    removeThumbnail(controlName: string): Promise<void>;
    ...
    getCurrentUser(): Promise<{ id: string; groups: string[] }>;
    getUserGroups(): Promise<{ id: string; name: string; desc: string }[]>;
    checkVisibility(visibilityConfig: {
        enabled?: boolean;
        groups?: string[];
        objectId?: string;
        condition?: '==' | '!=' | '>' | '<' | '>=' | '<=';
